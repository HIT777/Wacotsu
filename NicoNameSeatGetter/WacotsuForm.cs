...
namespace Wacotsu
{
	public partial class WacotsuForm : Form
	{
		...
		private async void init()
		{
			...
			var sessionCookie = browser.GetCookie(Niconico.LoginUnit.Uri, "user_session");
			if (sessionCookie == null)
			{
				throw new Exception("標準のブラウザでニコニコ動画にログインしてください");
			}
			var user = new Niconico.User { SessionId = sessionCookie.Value };
			this.wacotsu = new Wacotsu(user);
			...
			await Task.Run(() =>
			{
				try
				{
					foreach (var live in Niconico.LiveFetcher.SearchLives())
					{
		...
		private void wacotsu_Failed(object sender, FailedEventArgs e)
		{
			...
			switch (e.Reason)
			{
			case FailedReason.Closed:
				message = "すでに終了した放送です";
				break;

			case FailedReason.NoAuth:
				message = "この放送を視聴するには特別な認証が必要です";
				break;

			case FailedReason.NotFound:
				message = "放送が見つかりませんでした";
				break;

			case FailedReason.NotLogin:
				message = "ログイン状態が解除されていて放送の情報を取得できません。\r\n標準のブラウザでニコニコ動画にログインしてからもう一度このアプリを再起動してください";
				break;

			case FailedReason.RequireCommunityMember:
		...
		private void removeItemFromReservedListView(Niconico.Live live)
		{
			this.Invoke((MethodInvoker)delegate()
			{
				...
				{
					var currentLive = item.Tag as Niconico.Live;
		...
		/// <returns></returns>
		private ListViewItem createLiveListViewItem(Niconico.Live live, Niconico.Seat seat = null)
		{
		...
		private void listView_MouseDoubleClick(object sender, MouseEventArgs e)
		{
			try
			{
				...
				{
					var live = item.Tag as Niconico.Live;
					...
					if (listView == this.availableLiveListView)
					{
						...
						try
						{
							this.wacotsu.Reserve(live);
						}
						catch (Exception)
						{
							this.reservedLiveListView.Items.Remove(addItem);
							throw;
						}
					}
